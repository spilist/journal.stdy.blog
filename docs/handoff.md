...
- **현재 작업 슬라이스(미배포)** — 원복한 텍스트는 같은 편집 세션의 새 revision과
...
1. ~~배포~~ **완료 (2026-08-03).** 위 `Current State` 참조.
