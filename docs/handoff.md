...
`charness:impl` — **S2 배포와 폰 확인.** S2는 구현·리뷰가 끝났고 **배포만 남았다**
(`npm run deploy`). 그 뒤는 폰에서 볼 프로브 `P-6`·`P-7`이다.
...
1. **수용 확인 — 사람만 할 수 있다.** 절차는
   ...
   이게 가장 놓치기 쉬운 자리다.
2. **S2 배포.** `npm run deploy`. 그 다음 폰에서 `P-6`(30일 창에서 점을 짚을 수
   있는가) · `P-7`(그래프가 쓰는 자리 아래에 있는 게 맞는가). 단일 출처는
   [spec-first-slice.md](./spec-first-slice.md) `## Probe Questions`.
3. **`AC-19` 확인** — 폰에서 그래프 탭이 세로 스크롤과 싸우지 않는지.
   `pointerup` + 8px 슬롭으로 처리했지만 실사용은 다를 수 있다.
4. 남은 프로브 `P-2`·`P-3`.
