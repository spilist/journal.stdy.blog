...
`charness:quality` + 다중 렌즈 워크플로 — **공격적 품질 검사를 한 라운드 더 돈다.**
이 절만 읽고 바로 시작할 수 있게 아래에 필요한 것을 다 적었다. 사용자 확인을 다시
받지 말 것 — **핸드오프 멘션이 곧 이 라운드의 승인이다.**

### 먼저 알아야 할 것 — 2026-08-03에 무슨 일이 있었나

하루에 32개 파일 1100줄이 바뀌었고 **리뷰 라운드를 다섯 번 돌렸다.** 매 라운드가
**직전 라운드의 수정이 만든 새 결함**을 잡았다. 자초한 결함이 다섯이었고 그중 둘은
**재현된 글자 유실**(불변식 3)이었다. 다섯 다 **테스트가 없는 파일**에서 났다.

그래서 마지막에 방향을 바꿨다 — 리뷰를 한 번 더 도는 대신 **하네스를 만들었다.**
`state.svelte.js`·`gen-sw.mjs`·`worker/` 둘이 이제 테스트로 닿는다(도달 21개 중 10개).

**이번 라운드가 직전 다섯과 달라야 하는 점은 하나다: 지적을 리포트로 끝내지 말고
가능하면 그 자리에서 회귀 테스트로 고정할 것.** 하네스가 이미 있으므로 대부분
붙는다. 고칠 때는 **결함을 되돌려 넣어 테스트가 빨간불을 내는지 반드시 확인**하라 —
그 확인을 안 한 수정이 오늘 결함 다섯의 원인이다.
...
**아직 안 쓴 각도 후보**: 시간 축(첫 사용/1년/5년) · 여러 탭·기기 동시성 ·
되돌리기와 복구 경로 · 국제화와 입력기(IME) · 저장소 압박과 evict ·
문서와 코드의 인과 역전(주석이 거짓인 자리) · 의존성 업그레이드 내성.
...
- **`AC-22`** (applied 방향 사본) — 단위 테스트가 클라이언트·워커를 각각 고정했지만
...
사람이 하는 확인(`AC-12`·`AC-19`·`AC-22`·폰에서 `P-6`)은 아래 `Next Session`에 있고,
...
**2026-08-03: 오픈소스 공개 준비 + 품질 라운드 다섯 + 하네스 셋.** 남은 미결:
...
- ~~미해결 이슈 둘~~ **둘 다 닫혔다 (2026-08-03, 사용자와 논의).**
  ...
  내려받기 토스트가 그 개수를 말한다
- **`AC-11`의 applied 방향은 사람이 확인한 적이 없다** — `AC-22`로 신설했다.
  위 `Workflow Trigger`의 「아직 아무도 확인 못 한 것」 참조
- **테스트 135개 · 도달 21개 중 10개** ([기준선](../scripts/reach-baseline.txt)).
...
1. **수용 확인 — 사람만 할 수 있다.** 절차는
   ...
   그래프 탭) · `AC-22`(두 기기로 applied 방향 사본)가 남았다.**
   `AC-12`는 **S1과 같은 절차**다 (`S-2` 철회로 되돌아갔다).
   **`AC-22`가 가장 중요하다** — 2026-08-03에 새로 만든 경로이고 종단 확인이 없다.
...
5. **청크 push를 큰 가져오기로 시험** — 200일치쯤 지어낸 마크다운을 가져오고
