...
- **2026-07-27에 pull을 자동화했다** (미배포). 계약은
...
2. **pull 자동화를 배포하고 두 기기로 확인.** `npm run deploy`. 로컬에는 API가 없어
   동기화는 배포본에서만 보인다. 볼 것: 폰에서 고치고 PC 탭으로 돌아왔을 때 받아오는가 ·
   양쪽을 같이 고쳐놓고 돌아왔을 때 **분기 배너**가 뜨는가 · **올리기 직후 계속
   편집하다 앱을 갔다 와도 배너가 안 뜨는가**(메아리 회귀).
3. **폰에서 볼 것** — `P-6`(30일 창에서 점을 짚을 수 있는가). 단일 출처는
