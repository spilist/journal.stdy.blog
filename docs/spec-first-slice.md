...
- 전체 export는 `pinned.text`를 맨 위에 두고, 그 아래 날짜를 **내림차순**으로
  (sample.md와 같은 순서)
- **S2에서 여기에 날짜 범위가 붙었다.** 「전체」 export의 범위는 **그래프 창** 그
  자체다 (`S-2`) — 기본값이 최근 30일이고, 전량을 받으려면 그래프에서 「전체」를
  누른다. 조립 함수와 형식은 그대로다
...
- ~~**P-7.** (S2) 그래프가 쓰는 자리 아래에 있는 게 맞는가~~ **해소 (2026-07-26).**
  ...
  **반대 의견 (기록용).** 접혀 있으면 **창(=내려받기 범위) 선택기에 닿는 데 조작이
  하나 더 든다** — 전량 내려받기가 이제 「그래프 열기 → 전체 → 내려받기」 세 번이다.
  푸터 버튼 라벨이 범위를 계속 말하므로 함정은 아니지만, 이게 거슬리면 **범위 결합을
  떼어낸다**(`S-5` 1번). 그리고 접힘이 세션마다 초기화되므로, 매번 여는 게 귀찮으면
  열림 상태를 `journal`로 올린다 — `pinnedOpen`이 이미 그 형태다

## Deferred Decisions

- 주·월 회고 화면 — 그래프를 실제로 써본 뒤 판단
- ~~전체 export의 날짜 범위 선택 UI~~ **해소 (S2).** 새 UI를 만들지 않고 **그래프
  창을 범위로 쓴다** — 아래 `S-2. 내려받기 범위` 참조
...
**「전체 export의 날짜 범위 선택 UI」를 새로 만들지 않는다.** 그래프의 창이 이미
날짜 범위 선택기다 (설계 취향 1항).

- **경계 (2항):** 범위는 **그래프 창으로만** 정하고, 내려받기는 **버튼으로만** 한다
- 버튼 라벨이 범위를 말한다 (`최근 30일 내려받기` / `전체 240일 내려받기`).
  파일명에도 **창의 실제 양 끝**을 박는다
- **범위는 그래프가 그리는 날짜 배열 그 자체다.** `Journal.graphDates()` 하나가
  두 곳의 단일 출처고, 라벨도 `graphLabel()` 하나에서 나온다. 각자 계산하면
  보여준 구간과 파일이 조용히 어긋난다
- **「전체」는 양쪽에서 같은 뜻이어야 한다.** 그래서 창의 왼쪽 끝은 *점수*가 아니라
  *기록*이 있는 가장 이른 날이고, 오른쪽 끝은 오늘보다 뒤의 기록까지 덮는다.
  이게 어긋나면 「전체 내려받기」가 조용히 빠뜨린다
- **고정 블록은 범위와 무관하게 맨 위에 남는다** — 지금 서 있는 문장이지 그 기간의
  기록이 아니다
- 하루치 복사(`D13`)는 그대로다. 범위는 전체 쪽에만 붙는다
...
| **SC-12** | 내려받은 마크다운의 날짜 범위가 그래프 창과 같다 |
...
| **AC-18** | `unit` | `windowStart`/`windowEnd`가 창의 양 끝과 같다 — 모든 창 길이·경계에서. 이게 깨지면 파일과 그래프가 어긋난다. `SC-12` |
| **AC-20** | `unit` | 「전체」 범위로 자르면 기록이 하나도 안 빠진다 (로그만 쓴 날, 오늘 뒤의 날 포함). `SC-12` |
...
1. **내려받기 범위를 그래프 창에 묶은 것.** 푸터의 버튼이 **화면 아래쪽 그래프
   상태**에 달려 있다. 라벨로 완화했지만 진짜 대가는 오독이 아니다 —
   **전량 export가 한 번의 조작으로 닿지 않게 됐다.** S1에서는 버튼 하나면 전량이었고
   지금은 그래프에서 「전체」를 먼저 눌러야 한다. `D13`이 정한 「전체」의 뜻이
   기본값에서 바뀐 것이다. **거슬리면 라벨을 고정하고 범위를 떼어낸다** —
   `graphDates()`를 안 읽으면 끝이고 스키마는 안 바뀐다.
   **`P-7` 이후 이 대가가 한 칸 더 커졌다** — 그래프가 접혀 있어서 범위를 바꾸려면
   먼저 펼쳐야 한다.
