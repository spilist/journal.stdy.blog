...
- **AC-23** (신설 2026-08-03) 폰을 **비행기 모드**로 두고 어떤 블록을 고친다 → 그동안
  데스크톱에서 **같은 블록**을 고쳐 「올리기」 → 폰을 온라인으로 돌리고 「올리기」
  (폰이 나중 시각이라 이긴다) → **데스크톱에서 쓴 판본이 폰에 `⚠ 충돌 사본`으로
  남는가.** `SC-6`의 **applied 방향**이고, `AC-11`이 2026-07-26에 확인한 것은 거절
  방향뿐이다 — 이쪽은 사람이 돌려본 적이 없다
